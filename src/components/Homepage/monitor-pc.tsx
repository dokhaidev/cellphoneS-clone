...
import { monitorPCBrands, sampleMonitorsPCs } from "@/src/data/sampleMonitorsPCs";
import { Product } from "@/src/types/product";
import { ChevronLeft, ChevronRight } from "lucide-react";

export default function FeaturedMonitorsPCs() {
  ...
  const filteredProducts: Product[] =
    selectedBrand === "Xem tất cả"
      ? sampleMonitorsPCs
      : sampleMonitorsPCs.filter((product) => product.brand === selectedBrand);
    ...
    <main className="h-auto">
      <div className="max-w-[1250px] mx-auto px-4 pt-4">
        <div className="mb-6 flex items-center flex-row-reverse sm:flex-row justify-between">
          ...
          <BrandTabs
            brands={monitorPCBrands}
            selectedBrand={selectedBrand}
            onBrandSelect={(brand) => {
              setSelectedBrand(brand);
              setCurrentIndex(0); // Reset to first slide when brand changes
            }}
          />
        </div>
